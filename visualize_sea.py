...
weather_data = pd.read_csv('KSEA.csv', parse_dates=['date'])
...
with plt.style.context('style.mplstyle'):
    ...
    plt.bar(day_order, record_max_temps - record_min_temps, bottom=record_min_temps,
            edgecolor='none', color='#fed599', width=1)
    ...
    plt.bar(day_order, average_max_temps - average_min_temps, bottom=average_min_temps,
            edgecolor='none', color='#feb64d', width=1)
    ...
    plt.bar(day_order, actual_max_temps - actual_min_temps, bottom=actual_min_temps,
            edgecolor='grey', linewidth=0.5, color='#fe5d4d', width=1)
    ...
    plt.yticks(range(0, 105, 10), [r'{}$^\circ$'.format(x)
                                     for x in range(0, 105, 10)], fontsize=10)
    ...
    ax3 = ax1.twinx()
    plt.yticks(range(-5, 105, 10),[None])
    plt.ylim(-5, 105)
    plt.grid(False)

    plt.title('Seattle, SEA\'s weather, July 2014 - June 2015\n\n', fontsize=20)

    plt.savefig('seattle-weather.png')
